...
pub fn main(pose: &[[f32; 3]]) -> [Quaternion; 36] {
    ...
    let rotation_data: [Quaternion; 36] = [Quaternion::NAN; 36];
    let rotation_data = calc_rotation_data(&data, rotation_data);
...
fn calc_rotation_data(data: &[Vector3; 36], mut rotation_data: [Quaternion; 36]) -> [Quaternion; 36] {
...
mod test {
    ...
    fn impl_test() {
        ...
        assert!(data[35] != Vector3::ZERO);

        let rotation_data: [Quaternion; 36] = [Quaternion::NAN; 36];
        let _rotation_data = super::calc_rotation_data(&data, rotation_data);
        for idx in [33, 34, // torso & hips
            11, 12, 13, 14, 15, 16, // arms & legs
            23, 24, 25, 26, 27, 28,] {
            assert!(rotation_data[idx] != Quaternion::NAN);
        }
    }
}
